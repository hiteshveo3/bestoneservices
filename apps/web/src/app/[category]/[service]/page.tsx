...
import { approvedCleaningServicePages, approvedServicePages } from "@/content/approved-service-pages";
...
export async function generateMetadata({ params }: { params: Promise<{ category: string; service: string }> }): Promise<Metadata> {
  ...
  const approved = ({ ...approvedServicePages, ...approvedCleaningServicePages } as Record<string, (typeof approvedServicePages)[keyof typeof approvedServicePages]>)[`${category}/${service}`];
