...
export async function POST(request: Request) {
  ...
  const input = parsed.data; const db = getFirebaseDb(); const idempotency = db.collection("bookingIdempotency").doc(input.idempotencyKey);
  const result = await db.runTransaction(async (tx) => { const existing = await tx.get(idempotency); if (existing.exists) return existing.data() as { reference: string; id: string }; const id = randomUUID(); const reference = `BOS-${new Date().getUTCFullYear()}-${id.slice(0, 8).toUpperCase()}`; const now = new Date(); const quote = startingPrice(input); const booking = { ...input, id, reference, status: "New", startingPrice: quote, currency: "GBP", finalPriceConfirmed: false, createdAt: now, updatedAt: now, source: "web-guest" }; tx.set(db.collection("bookings").doc(id), booking); tx.set(db.collection("customers").doc(input.email.toLowerCase()), { email: input.email.toLowerCase(), fullName: input.fullName, updatedAt: now }, { merge: true }); tx.set(db.collection("bookingEvents").doc(), { bookingId: id, type: "created", fromStatus: null, toStatus: "New", actor: "guest", createdAt: now }); tx.set(idempotency, { id, reference, createdAt: now }); return { id, reference }; });
